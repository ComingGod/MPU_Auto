...
class Parser(object):
    ...
    def parse_root(self, root):
        ...
        if 'skips' in root.attrib:
            assert len(tr.skipped) == int(root.attrib['skips'])
        if 'tests' in root.attrib:
            assert len(list(ts)) == int(root.attrib['tests']) #+ int(root.attrib['skips']) + int(root.attrib['errors'])
