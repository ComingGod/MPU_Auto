...
class TestEmmcBoot:
    ...
    def test_emmc_boot_cm4_ddr(self):
        #download image
        self.image.download_image('test_emmc_boot_cm4_ddr', 'emmc', SDPimage = self.config['Image']['serial_download'], 
                                    bootImage1 = self.config['Image']['emmc_cm4_ddr_uboot'])
    ...
    def test_emmc_boot_cm4_ddr_dummy_ocram(self):
        #download image
        self.image.download_image('test_emmc_boot_cm4_ddr_dummy_ocram', 'emmc', SDPimage = self.config['Image']['serial_download'], 
    ...
    def test_emmc_boot_cm4_tcm_dummy_ddr(self):
        #download image
        self.image.download_image('test_emmc_boot_cm4_tcm_dummy_ddr', 'emmc', SDPimage = self.config['Image']['serial_download'], 
                                    bootImage1 = self.config['Image']['emmc_cm4_tcm_uboot_dummy_ddr'])
        #set boot device then reset
        self.bcu.set_boot_device('emmc')
        self.bcu.reset()
        #checking boot expectation
    ...
    def test_emmc_secondary_boot_cm4_tcm_dummy_ddr(self):
        #download image
        self.image.download_image('test_emmc_secondary_boot_cm4_tcm_dummy_ddr', 'emmc', SDPimage = self.config['Image']['serial_download'], 
                                    bootImage2 = self.config['Image']['emmc_cm4_tcm_uboot_dummy_ddr'])
        #set boot device then reset
        self.bcu.set_boot_device('emmc')
        self.bcu.reset()
        #checking boot expectation
    ...
    def test_emmc_no_image_enter_sdp(self):
        #clear all the images from each boot device
        self.image.download_image('test_emmc_no_image_enter_sdp', 'clear', SDPimage = self.config['Image']['serial_download'])
        #set boot device then reset
        self.bcu.set_boot_device('emmc')
        self.bcu.reset()
...
if __name__ == '__main__':
    ...
    subprocess.call([pytest, '-v' ,'-s', 'test_emmc_boot.py::TestEmmcBoot::test_emmc_boot_cm4_ddr' ])
    # pytest.main(['-v', '-s'])
